...
data %>%
  ...
  gather(key="key", value="value", -has_diabetes) %>% 
  filter(!is.na(has_diabetes)) %>%
  ggplot(aes(x=key, y=value, fill=has_diabetes)) +
...
```{r}
data <- data %>% select(-c('bp.2s', 'bp.2d'))
```
...
### Calculate Pearson's Correlation Coefficient between all variables

```{r}
ggcorr(data)
```

We interestingly see a strong correlation between the Stabilized Glucose Level and Glycosated Hemoglobin Level, our indicator for diabetes. We also notice a somewhat positive correlation for other factors above. We also notice a correlation between features in `weight`, `waist`, and `hip` measurements. This naturally makes sense.

Let us examine some interesting features more closely, colored by diabetes diagnosis:

```{r message=FALSE, warning=FALSE}
ggpairs(data, columns = c("glyhb", "waist", "weight", "stab.glu"), title = "Bivariate Analysis of Diabetes Features", upper = list(continuous = wrap("cor", size = 3)),
    lower = list(continuous = wrap("smooth",
        alpha = 0.7,
        size = 0.3)),
    mapping = aes(color = has_diabetes))
```
...
We observe a much wider spread in participants with diabetes than those without. There are, however, several very high outliers in the non-diabetic case and there is quite a bit of variation. May not be an accurate predictor. We can perform an ANOVA test to analyze the effect of the variance of this variable.
