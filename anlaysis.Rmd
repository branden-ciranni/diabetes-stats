...
logistic_model <- glm(has_diabetes ~ stab.glu + chol + hdl + ratio + weight_height_ratio + wh_ratio + age,data = no.na.train, family = binomial) %>%
  stepAIC(trace = TRUE, direction = 'backward')
summary(logistic_model)
```

The stepwise logistic regression with the missing values removed finds that stable glucose and cholesterol are the the only good predictors of diabetes.
```{r}
...
```
```{r}
predicted.classes <- ifelse(probabilities > 0.8, TRUE, FALSE)
...
logistic_model_missing <- glm(has_diabetes ~ stab.glu + chol, data = missing_train, family = binomial)
...
mean(predicted.classes == no.na.test$has_diabetes)
```
We see that the efficacy of the model is the same on the training set.